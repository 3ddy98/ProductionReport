...
def generate_line_report(prod_line,line_ID,hours_worked,total_employees,pdf):
	...
	prod_line['TimeEnd'] = prod_line['TimeEnd'].astype('datetime64[ns]')
	prod_line['TimeBeginning'] = prod_line['TimeBeginning'].astype('datetime64[ns]')
	prod_line['Workers']= total_employees
	prod_line['WPH'] = (prod_line['Boxes']/prod_line['RealDuration(Min)'])*60
	prod_line['WPH'] = prod_line['WPH'].astype('int32')
	prod_line['WPH/Person'] = prod_line['WPH']/total_employees
	prod_line['Boxes Produced'] = prod_line['Boxes'].cumsum()
	prod_line["TimeEnd"] = pd.to_datetime(prod_line["TimeEnd"].dt.strftime('%H:%M'),format="%H:%M")
	prod_line["TimeEnd"] = prod_line["TimeEnd"] - pd.Timedelta(hours=3)
	prod_line["TimeBeginning"] = pd.to_datetime(prod_line["TimeBeginning"].dt.strftime('%H:%M'),format="%H:%M")
	prod_line["TimeBeginning"] = prod_line["TimeBeginning"] - pd.Timedelta(hours=3)
	prod_line["TimeBeginning"] = prod_line["TimeBeginning"]
	timerange = pd.date_range( '1900-01-01 05:00:00','1900-01-01 18:00:00',freq="h")
	...
	date = str(prod_line['DateEnd'].min())
	title = line_ID + ' ' + str(date)[0:10]
	fig.suptitle(title)
	...
	ax1.set_xticks(timerange)
	ax1.set_xlim(min(timerange),max(timerange))
...
def main():
	#loading report
	print("Post-Production Reporter V0.1")
	...
	date = str(main_df['DateBeginning'].median())[0:10]
